...
void generateGS(char* b, int m, int* suffix, int* prefix) {
  ...
  for (int i = 0; i < m - 1; ++i) { // b[0, i]
    ...
    if (j == -1) prefix[k] = 1;                         // 如果公共后缀子串也是模式串的前缀子串
  }
}
...
int main()
{
    char *s1="abcacabcbcbacabc",*s2="cbacabc";
    int result = bm(s1,16,s2,7) ;
    printf("%d",result);
}
